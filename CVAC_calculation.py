	...
	#Parse mpileup file for chromosome, position, and ref and depth 
		...
		if (chrom==chrom_p and int(pos)==int(pos_p) and str(ref_vcf)==str(ref)):
			...
			if (int(c)!=0):
				...
				my_row=[chrom, pos, ref_vcf, alt_vcf, int(c1), int(c), int(denom), rna_comp_vaf, abs(1.0-round(float(rna_comp_vaf),2)), sample_name]
				...
				if ((c==0) and float(denom)!=0.0):
					...
					my_row=[chrom, pos, ref_vcf, alt_vcf, c1, 0, int(denom), 0.0, round((float(c1)/float(denom)), 2), sample_name]
					my_RNA_VAF_file.writerow(my_row)
